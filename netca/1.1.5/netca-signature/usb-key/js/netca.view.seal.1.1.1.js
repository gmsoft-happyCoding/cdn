...
var NetcaSignAPI = (function () {
          ...
          if (dataType.indexOf("batchSign") !== -1) {
            layer.confirm(
              ...
              function (key, layero) {
                ...
                for (var index = 0; index < len; index++) {
                  if (
                    $(".vSealBoxDelete").eq(index).attr("data-type") ===
                    dataType
                  ) {
                    arr.push(
                      parseInt(
                        $(".vSealBoxDelete").eq(index).attr("id").substring(14)
                      )
                    );
                  }
                }
            ...
            for (var j = 0; j < len; j++) {
              if ($(".vSealBoxDelete").eq(j).attr("data-type") === dataType) {
                arr.push(
                  parseInt($(".vSealBoxDelete").eq(j).attr("id").substring(14))
                );
              }
            }
            for (var k = 0; k < arr.length; k++) {
  ...
  // 根据样式 产生新图片
  // 注意：文本位置调整改widthPercentage和heightPercentage
  function graphicsProcessDrawText(_srcBytes, appearanceObj, callbackFn) {
    var params = {
      srcFile: "", //源图片文件，允许为空
      srcBytes: _srcBytes, //源图片文件的Base64编码，不能为空
      destFile: "", //目标文件，允许为空
  ...
  that.onSave = function () {
    if (saveChartletArray.length > 0) {
      ...
      if (urlParams.appearance) {
        graphicsProcessDrawText(
          saveChartletArray[0].sealImageEncode,
          urlParams.appearance,
          function (newImgEncode) {
            for (var i = 0; i < saveChartletArray.length; i++) {
              params.SignDisposables[i].sealImageEncode = newImgEncode;
            }
            sign(params);
          }
        );
    ...
    reader.onload = function (event) {
      ...
      var interval = setInterval(function () {
        if (globalPDFViewerApplication.pdfDocument == null) {
          ...
          globalPDFViewerApplication.pdfDocument
            .getData()
            .then(function (data) {
              var params = {
  ...
  that.netcaPDFClickEvent = function (signObj, res, x, y, w, h) {
    ...
    $("#viewer").click(function (e) {
      ...
      if (flag > 0) {
        if (signObj.batchSignParams) {
          var startPageNum =
            signObj.batchSignParams.pageType === 0
              ? signObj.batchSignParams.startPageNum
              : 1;
          for (
            let index = startPageNum;
        ...
        if (
          ...
          if (signObj.batchSignParams) {
            ...
            if (signObj.batchSignParams.pageType === 1) {
              for (
                let index = 1;
                index < globalPDFViewerApplication.pagesCount + 1;
                index++
              ) {
                if (index % 2 !== 0) {
                  saveChartletArray.push({
  ...
  var CHARTLET_SEAL_STRATEGY = {
    singleSeal: function (params) {
      ...
      var goSingleSeal = function () {
        ...
        if (signPositionXPos || signPositionYPos) {
          alert(
            "第" +
              params.signPosition.pageNum +
              "页的印章超出页面范围，请重新选择盖章位置"
          );
          return;
        }
        var newscale = NetcaSignAPI.getScale(); //缩放比例
    ...
    acrossPageSeal: function (params) {
      var goAcrossSeal = function () {
        if (
          ...
          params.acrossPageSeal.endPageNum >
            globalPDFViewerApplication.pagesCount
        ) {
          alert("传入页码有误");
          return;
        }
      ...
      // 如果印章图片的宽高有一个设置为0，就不取传入的宽高
      if (
        params.acrossPageSeal.width === 0 ||
        params.acrossPageSeal.height === 0
      ) {
        // 判断图片原文中是否包含宽高信息
        var tempStr = atob(params.sealImageEncode);
        var heightReg = /(?<="Height"\s*:\s*").*?(?=cm")/;
    ...
    batchSeal: function (params) {
      var goBatchSeal = function () {
        ...
        var num =
          params.batchSeal.pageType === 0 ? params.batchSeal.startPageNum : 1;
        for (
          let index = num;
        ...
        // 如果不包含，就使用传入的证书开始读取key中印章信息
          ...
          NetcaPKI.readSeal({
            ...
            // 有普通印章
              ...
              if (
                params.batchSeal.xPos !== undefined &&
                params.batchSeal.yPos !== undefined
              ) {
                goBatchSeal();
              } else {
                alert("没有提供坐标信息");
      ...
      for (var i = 0; i < saveChartletArray.length; i++) {
        ...
        var deleteClick = function (event) {
          ...
          if (dataType.indexOf("batchSign") !== -1) {
            ...
            for (var index = 0; index < len; index++) {
              if (
                $(".vSealBoxDelete").eq(index).attr("data-type") === dataType
              ) {
                arr.push(
                  parseInt(
                    $(".vSealBoxDelete").eq(index).attr("id").substring(14)
