...
var NetcaSignAPI = (function () {
  ...
  that.initUI = function () {
    if (NetcaUtils.isUserManualSignUI()) {
      ...
      /**选中option，缩放pdf*/
      document.getElementById("scaleSelect").addEventListener("change", that.zoomPDF);
      //绑定事件
      ...
      document.getElementById("netcainvalidClose").addEventListener("click", function () {
        that.closeSignedDataVerifyInfo("netcainvalidClose");
      });
      // 关闭按钮绑定函数-仅关闭验证框
      document.getElementById("netcaclose2").addEventListener("click", function () {
      ...
      // 旋转按钮
      document.getElementById("leftBtn").addEventListener("click", that.onRotateLeft);
      document.getElementById("rightBtn").addEventListener("click", that.onRotateRight);

      //保存按钮 特供重庆大家项目
      document.getElementById("netcasave").addEventListener("click", that.onSave);
    ...
    if (saveChartletArray.length > 0) {
      for (var i = 0; i < saveChartletArray.length; i++) {
        ...
        var deleteClick = function (event) {
          ...
          var index = Number(event.target.id.substring(14));
          var dataType = $("#vSealBoxDelete" + index).attr("data-type");
          if (dataType.indexOf("batchSign") !== -1) {
            layer.confirm(
              "你想要删除当前页还是所有页印章信息？",
              {
                title: "询问",
              ...
              function (key, layero) {
                //按钮【按钮一】的回调
                var len = $(".vSealBoxDelete").length;
                var arr = [];
                for (var index = 0; index < len; index++) {
                  if ($(".vSealBoxDelete").eq(index).attr("data-type") === dataType) {
                    arr.push(parseInt($(".vSealBoxDelete").eq(index).attr("id").substring(14)));
                  }
                }
                for (var k = 0; k < arr.length; k++) {
                  // 界面上删去该贴图
                  $("#vSeal" + arr[k]).remove();
                  $("#vSealBox" + arr[k]).remove();
                  $("#vSealBoxDelete" + arr[k]).remove();
                  // 逻辑上删去该贴图
                  for (var i = 0; i < saveChartletArray.length; i++) {
                    if (saveChartletArray[i].id === arr[k]) {
                      saveChartletArray.splice(i, 1);
                      break;
                    }
                  }
                }
              ...
              function () {
                ...
                for (var k = 0; k < saveChartletArray.length; k++) {
                  if (saveChartletArray[k].id === index) {
                    saveChartletArray.splice(k, 1);
                    break;
                  }
                }
              }
            );
          } else if (dataType.indexOf("acrossSign") !== -1) {
            var len = $(".vSealBoxDelete").length;
            var arr = [];
            for (var j = 0; j < len; j++) {
              if ($(".vSealBoxDelete").eq(j).attr("data-type") === dataType) {
                arr.push(parseInt($(".vSealBoxDelete").eq(j).attr("id").substring(14)));
              }
            }
            for (var k = 0; k < arr.length; k++) {
              // 界面上删去该贴图
              ...
              $("#vSealBoxDelete" + arr[k]).remove();
              // 逻辑上删去该贴图
              for (var n = 0; n < saveChartletArray.length; n++) {
                if (saveChartletArray[n].id === arr[k]) {
                  saveChartletArray.splice(n, 1);
                  break;
                }
              }
            }
            ...
            // 逻辑上删去该贴图
            for (var i = 0; i < saveChartletArray.length; i++) {
              if (saveChartletArray[i].id === index) {
                saveChartletArray.splice(i, 1);
                break;
              }
            }
          ...
          var read = function (streamId, res) {
            ...
            NetcaPKI.readStream(params)
              .Then(function (res) {
                ...
                var dest = new Uint8Array(data.byteLength + destFile.byteLength);
                dest.set(destFile);
  ...
  that.selectSealSigntureInfoWithDispaly = function (signObj) {
    ...
    NetcaPKI.SelectSealSigntureInfoWithDispaly(params)
      .Then(function (res) {
        $.unblockUI();
        NetcaSignAPI.selectSealSigntureInfoWithDispalySuccessCallBack(res, signObj);
      })
      .Catch(function (res) {
        $.unblockUI();
  ...
  that.selectSealSigntureInfoWithDispalySuccessCallBack = function (res, signObj) {
    $("#sign").html('<img src="ui/images/icon/but_signature.png" alt=""><br>取消');
    ...
    // 骑缝章
    if (signObj.acrosspageSignParams) {
      signObj.acrosspageSignParams.startPageNum = res.acrosspageSignParams.startPageNum; // 骑缝章
    ...
    if (signObj.keywordSignParams) {
      ...
      signObj.keywordSignParams.startPageNum = res.KeywordSignParams.startPageNum; //PDF文档的开始页码
  ...
  that.setSignImageSize = function (imageObj) {
    ...
    if (imageObj) {
      ...
      $("#NetcaSignMoveImage").attr("src", "data:image/PNG;base64," + imageObj.sealImageEncode);
    } else {
      ...
      if (click_x > 0 && viewer_width - click_x - blank_left >= blank_left) {
        ...
        var deleteClick = function (event) {
          ...
          if (freeSealParams.length > 0) {
            ...
            document.querySelector(".vSealBoxDelete").addEventListener("click", deleteClick);
          } else {
            that.hideSignImage();
            document.getElementById("viewer").onclick = null;
            $("#netcasignedDataVerify").css("display", "none");
  ...
  that.openPDFUrl = function (urlparams, callback) {
    ...
    if (urlparams.cookie && urlparams.cookie !== "" && urlparams.cookie !== null) {
  ...
  that.netcaPDFClickEvent = function (signObj, res, x, y, w, h) {
    ...
    $("#viewer").click(function (e) {
      ...
      if (flag > 0) {
        if (signObj.batchSignParams) {
          var startPageNum =
            signObj.batchSignParams.pageType === 0 ? signObj.batchSignParams.startPageNum : 1;
          for (let index = startPageNum; index < signObj.batchSignParams.endPageNum + 1; index++) {
            if (
              click_x < 0 ||
              click_x >
                $("#viewer .page")
              ...
              alert("第" + index + "页的印章超出页面范围，请重新选择盖章位置");
              that.hideSignImage();
              $("#sign").html('<img src="ui/images/icon/but_signature.png" alt=""><br>签章');
              return;
            }
        ...
        if (
          ...
          if (signObj.batchSignParams) {
            // 页码范围
            if (signObj.batchSignParams.pageType === 0) {
              for (
                let index = signObj.batchSignParams.startPageNum;
                index < signObj.batchSignParams.endPageNum + 1;
                index++
              ) {
                saveChartletArray.push({
                  id: chartletNum,
                  pageNum: index,
                  xPos: parseInt((scale / newscale) * (click_x - sealImage_halfWidth)),
                  yPos: parseInt((scale / newscale) * (click_y - sealImage_halfheight + 4)),
                  width: parseInt(originImage_width), //签名域/签章的宽度
                  height: parseInt(originImage_height), //签名域/签章的高度
                  sealImageEncode: sealImageSrc,
                  type: "batchSign" + group,
                });
                chartletNum++;
              }
              group++;
            ...
            // 奇数页
            if (signObj.batchSignParams.pageType === 1) {
              for (let index = 1; index < globalPDFViewerApplication.pagesCount + 1; index++) {
                if (index % 2 !== 0) {
                  saveChartletArray.push({
                    id: chartletNum,
                    pageNum: index,
            ...
            // 偶数页
            if (signObj.batchSignParams.pageType === 2) {
              for (let index = 1; index < globalPDFViewerApplication.pagesCount + 1; index++) {
                if (index % 2 === 0) {
                  saveChartletArray.push({
                    id: chartletNum,
          ...
          } else if (signObj.acrosspageSignParams) {
            that.getAcrossPageSeal(
              signObj,
              click_x - sealImage_halfWidth,
              click_y - sealImage_halfheight,
              w,
              h
            );
            return false;
  ...
  that.getAcrossPageSeal = function (signObj, x, y, w, h) {
    $.blockUI(blockUIConfig("正在生成骑缝章，请稍等..."));
    var newscale = that.getScale(); //缩放比例
    var scale = 0.75; // 75% 对应72dpi; 如果不是75%缩放比例，需要对应转换为72dpi的坐标大小
    var K = scale / newscale;
    var params = {
      srcStreamId: unfinishedStreamId,
      sealImage: sealImageSrc, // 印章图片
      startPage: signObj.acrosspageSignParams.startPageNum, // 起始页，以1为开始，-1表示最后一页
      endPage: signObj.acrosspageSignParams.endPageNum, // 结束页，以1为开始，-1表示最后一页
      x: parseInt(x * K), // 签章位置横坐标
      y: parseInt(y * K), // 签章位置纵坐标
      imageWidth: parseInt(w * K), // 印章宽度
      imageHeight: parseInt(h * K), // 印章高度
    };
    NetcaPKI.getAcrossPageSeal(params)
      .Then(function (res) {
        if (res && res.pages && res.pages.length) {
          for (var index = 0; index < res.pages.length; index++) {
            saveChartletArray.push({
              id: chartletNum,
              pageNum: res.pages[index].pageNum,
              xPos: res.pages[index].x,
              yPos: res.pages[index].y,
              width: res.pages[index].width, //签名域/签章的宽度
              height: res.pages[index].height, //签名域/签章的高度
              sealImageEncode: res.pages[index].image,
              type: "acrossSign" + group,
            });
            chartletNum++;
          }
          group++;
  ...
  that.freeSeal = function () {
    ...
    var scale = 0.75; // 75% 对应72dpi; 如果不是75%缩放比例，需要对应转换为72dpi的坐标大小
    var _xPos = parseInt(
      (scale / newscale) * (freeSealParams[freeLength].click_x - sealImage_halfWidth)
    ); //签名域/签章左下角的水平向右方向坐标
    var _yPos = parseInt(
      (scale / newscale) * (freeSealParams[freeLength].click_y - sealImage_halfheight)
    ); //签名域/签章左下角的垂直向上方向坐标
  ...
  that.seal_SignSealPosition = function (signObj, _pageNum, _type) {
    ...
    var params = {
      ...
      SignPosition: {
        ...
        height: parseInt(originImage_height) || signObj.signPositionParams.height, //签名域/签章的高度
      },
      //时间戳对象
      Tsa: {
        tsaUrl: timeStamp.tsaUrl, //时间戳地址
      ...
      .Then(function (res) {
        ...
        if (signObj.keywordSignParams.index == 1) {
          ...
          var page_width = $("#viewer .page")
            ...
            .width();
          var page_height = $("#viewer .page").eq(res.pages[0].pageNum - 1)[0].offsetTop;
          // 如果这个章所在页比外部框小，说明还有边缘空白部分，需要计算
          if (viewer_width > page_width) {
            var blank_left = (viewer_width - page_width) / 2;
          } else {
          ...
          var page_width = $("#viewer .page")
            ...
            .width();
          var page_height = $("#viewer .page").eq(res.pages[length].pageNum - 1)[0].offsetTop;
          // 如果这个章所在页比外部框小，说明还有边缘空白部分，需要计算
          if (viewer_width > page_width) {
            var blank_left = (viewer_width - page_width) / 2;
          } else {
          ...
          if (signObj.keywordSignParams.offsetX) {
            res.pages[length].keywords[length2].x += signObj.keywordSignParams.offsetX;
          }
          if (signObj.keywordSignParams.offsetY) {
            res.pages[length].keywords[length2].y += signObj.keywordSignParams.offsetY;
          }
          res.pages[length].keywords[length2].y -= signObj.keywordSignParams.height / 2;
          ...
          for (var i = 0; i < res.pages.length; i++) {
            var page_width = $("#viewer .page")
              ...
              .width();
            var page_height = $("#viewer .page").eq(res.pages[i].pageNum - 1)[0].offsetTop;
            // 如果这个章所在页比外部框小，说明还有边缘空白部分，需要计算
            if (viewer_width > page_width) {
              var blank_left = (viewer_width - page_width) / 2;
            } else {
            ...
            for (var j = 0; j < res.pages[i].keywords.length; j++) {
              ...
              res.pages[i].keywords[j].y -= signObj.keywordSignParams.height / 2;
  ...
  that.SignatureCreatorSuccessReadStreamUpdateCallBack = function (res, signSuccessRes, index) {
    ...
    if (!res.hasMoreData) {
      that.SignatureCreatorCallBack(signSuccessRes, res.destFileStream.streamId);
      return;
    }
    // 读10M
    var params = {
  ...
  //创建数据流
  that.createStreamCallBack = function (len, signParams, saveType, openWay, obj) {
    $("#netcasignedDataVerify").css("display", "block");
  ...
  // 写入数据流
  that.writeStreamCallBack = function (res, signParams, len, saveType, openWay, obj) {
    ...
    for (var i = 0; i < btn_fold.length; i++) {
      ...
      _btn_fold.onclick = function () {
        if ($(".netcasignTime").eq(this.index).css("display") === "none") {
          ...
          $(".btn_fold_img").eq(this.index).attr("src", "ui/images/icon/btn_fold.png");
        } else {
          $(".netcasignTime").eq(this.index).css({ display: "none" });
          $(".btn_fold_img").eq(this.index).attr("src", "ui/images/icon/btn_unfold.png");
        }
        _btn_fold = null;
      };
    }
  ...
  that.getSealConfigInfoSuccessCallBack = function (res, verifyLevel, streamId) {
  ...
  that.createRevokeEvent = function (_id, streamId) {
    // 判断是否能获取到文档内容
    // 如果能获取到文档内容且页数大于0
    if (globalPDFViewerApplication.pdfDocument != null && $("#viewer .page").length > 0) {
      // 获取章信息数组总长度
  ...
  that.revokeSignatureSuccessCall = function (res, streamId, updateRes) {
    ...
    if (NetcaPDFSeal.mode == 2) {
      ...
      if (NetcaPDFSeal.getSignatureCount() > 0) {
        sealFileId = updateRes.destFileStream ? updateRes.destFileStream.streamId : null;
        hasSign = true;
      } else {
        // 设置为未签状态
        hasSign = false;
  ...
  that.SignatureVerifierUndoPDFReadStreamUpdateCallBack = function (res, signSuccessRes) {
    ...
    signSuccessRes.destFileEncode.push(res.data);
    if (!res.hasMoreData) {
      that.revokeSignatureSuccessCall(signSuccessRes, res.destFileStream.streamId, signSuccessRes);
      return;
    }
    var params = {
      streamId: res.destFileStream.streamId,
...
var NetcaIESignAPI = (function () {
  ...
  that.openPDF = function (byteBase64, callback) {
    if (readerAx == null) readerAx = document.getElementById("NetcaReaderAx").object;
    var comObj = null;
    try {
...
var NetcaPDFSeal = (function () {
   ...
   * 获取签章文件的base64编码
   */
  that.MultiFile.getSealSignFile = function (sealFileId, succcallback, failback) {
    if (/\.pdf$/gi.test(sealFileId)) {
      ...
      NetcaPKI.readSealStream(params)
        .Then(function (res) {
          that.readStreamWithGetSealSignFile(res, sealFileId, succcallback, failback);
        })
        .Catch(function (res) {
  ...
  // 递归读取文件数据
  that.readStreamWithGetSealSignFile = function (res, sealFileId, succcallback, failback) {
    if (!res.hasMoreData) {
  ...
  var CHARTLET_SEAL_STRATEGY = {
    singleSeal: function (params) {
      ...
      var goSingleSeal = function () {
        var signPositionXPos =
          params.signPosition.xPos < 0 ||
          params.signPosition.xPos >
            $("#viewer .page")
              .eq(params.signPosition.pageNum - 1)
              .width();
        var signPositionYPos =
          params.signPosition.yPos < 0 ||
          params.signPosition.yPos >
            $("#viewer .page")
              .eq(params.signPosition.pageNum - 1)
              .height();
        if (signPositionXPos || signPositionYPos) {
          alert("第" + params.signPosition.pageNum + "页的印章超出页面范围，请重新选择盖章位置");
          return;
        }
        var newscale = NetcaSignAPI.getScale(); //缩放比例
        var scale = 0.75; // 75% 对应72dpi; 如果不是75%缩放比例，需要对应转换为72dpi的坐标大小
        ...
        var page_width = $("#viewer .page")
          .eq(params.signPosition.pageNum - 1)
          .width();
        var page_height = $("#viewer .page").eq(params.signPosition.pageNum - 1)[0].offsetTop;
        // 如果这个章所在页比外部框小，说明还有边缘空白部分，需要计算
        if (viewer_width > page_width) {
          var blank_left = (viewer_width - page_width) / 2;
        } else {
      ...
      if (params.signPosition.xPos === undefined || params.signPosition.yPos === undefined) {
        alert("没有提供坐标");
        return;
      }
      ...
      if (params.signPosition.width === 0 || params.signPosition.height === 0) {
        ...
        // 如果包含，则取出宽高信息
        if (tempStr.match(heightReg) !== null && tempStr.match(widthReg) !== null) {
          params.signPosition.width = Number(tempStr.match(widthReg)[0]) * 28.346;
          params.signPosition.height = Number(tempStr.match(heightReg)[0]) * 28.346;
          goSingleSeal();
        }
              ...
              if (res.seSeals.length > 1) {
                ...
                res.seSeals.forEach(function (item) {
                  NetcaPKI.sesStampDecode({
                    ...
                    // 取其中的宽和高
                    if (res.sesStamp.sealInfo.picture.data === params.sealImageEncode) {
                      flag = true;
                      if (res.sesStamp.sealInfo.picture.width !== 0) {
                        params.signPosition.width = res.sesStamp.sealInfo.picture.width * 2.8346;
                      }
                      if (res.sesStamp.sealInfo.picture.height !== 0) {
                        params.signPosition.height = res.sesStamp.sealInfo.picture.height * 2.8346;
                      }
                      goSingleSeal();
                    }
                  });
    ...
    keywordSeal: function (params) {
      ...
      if (params.sealKeyWord.width === 0 || params.sealKeyWord.height === 0) {
        ...
        // 如果包含，则取出宽高信息
        if (tempStr.match(heightReg) !== null && tempStr.match(widthReg) !== null) {
          params.sealKeyWord.width = Number(tempStr.match(widthReg)[0]) * 28.346;
          params.sealKeyWord.height = Number(tempStr.match(heightReg)[0]) * 28.346;
          goKeywordSeal();
        }
        // 如果不包含，就使用传入的证书开始读取key中印章信息
        else {
                ...
                res.seSeals.forEach(function (item) {
                  NetcaPKI.sesStampDecode({
                    ...
                    if (res.sesStamp.sealInfo.picture.data === params.sealImageEncode) {
                      flag = true;
                      if (res.sesStamp.sealInfo.picture.width !== 0) {
                        params.sealKeyWord.width = res.sesStamp.sealInfo.picture.width * 2.8346;
                      }
                      if (res.sesStamp.sealInfo.picture.height !== 0) {
                        params.sealKeyWord.height = res.sesStamp.sealInfo.picture.height * 2.8346;
                      }
                      goKeywordSeal();
                    }
                  });
    ...
    acrossPageSeal: function (params) {
      var goAcrossSeal = function () {
        if (
          Number.isNaN(params.acrossPageSeal.endPageNum) ||
          Number.isNaN(params.acrossPageSeal.startPageNum) ||
          params.acrossPageSeal.startPageNum < 0 ||
          params.acrossPageSeal.endPageNum > globalPDFViewerApplication.pagesCount
        ) {
          alert("传入页码有误");
          return;
        }
        for (
          let index = params.acrossPageSeal.startPageNum;
          index < params.acrossPageSeal.endPageNum + 1;
          index++
        ) {
          if (
            params.acrossPageSeal.yPos < 0 ||
            params.acrossPageSeal.yPos >
              $("#viewer .page")
                .eq(index - 1)
                .height()
          ) {
        ...
        var params1 = {
          srcStreamId: NetcaSignAPI.getFileStreamId(),
          sealImage: params.sealImageEncode, // 印章图片
          startPage: params.acrossPageSeal.startPageNum, // 起始页，以1为开始，-1表示最后一页
          endPage: params.acrossPageSeal.endPageNum, // 结束页，以1为开始，-1表示最后一页
          x: params.acrossPageSeal.xPos, // 签章位置横坐标
          y: params.acrossPageSeal.yPos, // 签章位置纵坐标
          imageWidth: params.acrossPageSeal.width, // 印章宽度
          imageHeight: params.acrossPageSeal.height, // 印章高度
        };
        $.blockUI(blockUIConfig("正在生成骑缝章，请稍等..."));
        NetcaPKI.getAcrossPageSeal(params1)
          .Then(function (res) {
            if (res && res.pages && res.pages.length) {
              for (var index = 0; index < res.pages.length; index++) {
                newSaveChartletArray.push({
      ...
      // 如果印章图片的宽高有一个设置为0，就不取传入的宽高
      if (params.acrossPageSeal.width === 0 || params.acrossPageSeal.height === 0) {
        // 判断图片原文中是否包含宽高信息
        var tempStr = atob(params.sealImageEncode);
        var heightReg = /(?<="Height"\s*:\s*").*?(?=cm")/;
        var widthReg = /(?<="Width"\s*:\s*").*?(?=cm")/;
        // 如果包含，则取出宽高信息
        if (tempStr.match(heightReg) !== null && tempStr.match(widthReg) !== null) {
          params.acrossPageSeal.width = Number(tempStr.match(widthReg)[0]) * 28.346;
          params.acrossPageSeal.height = Number(tempStr.match(heightReg)[0]) * 28.346;
          goAcrossSeal();
        }
        // 如果不包含，就使用传入的证书开始读取key中印章信息
        else {
          ...
          NetcaPKI.readSeal({
            ...
            if (res.seSeals !== null) {
              if (res.seSeals.length > 1) {
                ...
                res.seSeals.forEach(function (item) {
                  NetcaPKI.sesStampDecode({
                    ...
                    if (res.sesStamp.sealInfo.picture.data === params.sealImageEncode) {
                      flag = true;
                      if (res.sesStamp.sealInfo.picture.width !== 0) {
                        params.acrossPageSeal.width = res.sesStamp.sealInfo.picture.width * 2.8346;
                      }
                      if (res.sesStamp.sealInfo.picture.height !== 0) {
                        params.acrossPageSeal.height =
                ...
                NetcaPKI.sesStampDecode({
                  ...
                  if (res.sesStamp.sealInfo.picture.height !== 0) {
                    params.acrossPageSeal.height = res.sesStamp.sealInfo.picture.height * 2.8346;
                  }
                  goAcrossSeal();
                });
              }
            ...
            // 有普通印章
              ...
              params.acrossPageSeal.width = 119;
              params.acrossPageSeal.height = 119;
              if (
                params.acrossPageSeal.xPos !== undefined &&
                params.acrossPageSeal.yPos !== undefined
              ) {
                goAcrossSeal();
              } else {
                alert("没有提供坐标信息");
    ...
    batchSeal: function (params) {
      var goBatchSeal = function () {
        if (
          Number.isNaN(params.batchSeal.endPageNum) ||
          Number.isNaN(params.batchSeal.startPageNum) ||
          params.batchSeal.startPageNum < 0 ||
          params.batchSeal.endPageNum > globalPDFViewerApplication.pagesCount
        ) {
          alert("传入页码有误");
          return;
        }
        var num = params.batchSeal.pageType === 0 ? params.batchSeal.startPageNum : 1;
        for (let index = num; index < params.batchSeal.endPageNum + 1; index++) {
          var x =
            params.batchSeal.xPos < 0 ||
            params.batchSeal.xPos >
              $("#viewer .page")
                .eq(index - 1)
                .width();
          var y =
            params.batchSeal.yPos < 0 ||
            params.batchSeal.yPos >
              $("#viewer .page")
                .eq(index - 1)
                .height();
          if (x || y) {
        ...
        // 置入证书
        NetcaSignAPI.setCertEncode(params.certEncode);
        if (params.batchSeal.pageType === 0) {
          for (
            var index = params.batchSeal.startPageNum;
            index < params.batchSeal.endPageNum + 1;
            index++
          ) {
            newSaveChartletArray.push({
              id: chartletNum,
              pageNum: index,
              xPos: parseInt((scale / newscale) * params.batchSeal.xPos),
              yPos: parseInt((scale / newscale) * params.batchSeal.yPos),
              width: parseInt(params.batchSeal.width), //签名域/签章的宽度
              height: parseInt(params.batchSeal.height), //签名域/签章的高度
              sealImageEncode: params.sealImageEncode,
              type: "batchSign" + group,
            });
            chartletNum++;
          ...
          group++;
        } else if (params.batchSeal.pageType === 1) {
          for (var index = 1; index < globalPDFViewerApplication.pagesCount + 1; index++) {
            if (index % 2 !== 0) {
              newSaveChartletArray.push({
                id: chartletNum,
                pageNum: index,
          ...
          group++;
        } else if (params.batchSeal.pageType === 2) {
          for (let index = 1; index < globalPDFViewerApplication.pagesCount + 1; index++) {
            if (index % 2 === 0) {
              newSaveChartletArray.push({
                id: chartletNum,
      ...
      // 如果印章图片的宽高有一个设置为0，就不取传入的宽高
      if (params.batchSeal.width === 0 || params.batchSeal.height === 0) {
        // 判断图片原文中是否包含宽高信息
        var tempStr = atob(params.sealImageEncode);
        var heightReg = /(?<="Height"\s*:\s*").*?(?=cm")/;
        var widthReg = /(?<="Width"\s*:\s*").*?(?=cm")/;
        // 如果包含，则取出宽高信息
        if (tempStr.match(heightReg) !== null && tempStr.match(widthReg) !== null) {
          params.batchSeal.width = Number(tempStr.match(widthReg)[0]) * 28.346;
          params.batchSeal.height = Number(tempStr.match(heightReg)[0]) * 28.346;
          goBatchSeal();
        }
        // 如果不包含，就使用传入的证书开始读取key中印章信息
        else {
          ...
          NetcaPKI.readSeal({
                ...
                res.seSeals.forEach(function (item) {
                  NetcaPKI.sesStampDecode({
                    ...
                    if (res.sesStamp.sealInfo.picture.data === params.sealImageEncode) {
                      flag = true;
                      if (res.sesStamp.sealInfo.picture.width !== 0) {
                        params.batchSeal.width = res.sesStamp.sealInfo.picture.width * 2.8346;
                      }
                      if (res.sesStamp.sealInfo.picture.height !== 0) {
                        params.batchSeal.height = res.sesStamp.sealInfo.picture.height * 2.8346;
                      }
                      goBatchSeal();
                    }
                  });
            ...
            // 有普通印章
              ...
              params.batchSeal.width = 119;
              params.batchSeal.height = 119;
              if (params.batchSeal.xPos !== undefined && params.batchSeal.yPos !== undefined) {
                goBatchSeal();
              } else {
                alert("没有提供坐标信息");
              }
