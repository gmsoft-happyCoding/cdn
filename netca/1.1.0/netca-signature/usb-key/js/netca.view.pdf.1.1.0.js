...
var NetcaSignAPI = (function(){
    ...
    NetcaSignAPI.isInsertKey = function(){
        ...
        netca_isInsertKey('','','',NetcaSignAPI.InsertCallBack,NetcaSignAPI.UnInsertCallBack);
    ...
    NetcaSignAPI.InsertCallBack = function(res){
        ...
        if(res.insertCount){
            netca_SelectSealSigntureInfoWithDispaly(function(res){
                if(res.type === 2){
    ...
    NetcaSignAPI.zoomPDFBySelectOptionScale = function(){
        ...
        netca_scaleSelect.onchange = function(){
            var result = netca_scaleSelect.options[netca_scaleSelect.selectedIndex].value;
    ...
    * */
    ...
    NetcaSignAPI.netcaPdfClickEvent = function(){
        ...
        $('#netcaPDF').click(function(e){
            ...
            var blank_width = parseInt($('canvas').eq(page_size-1).css('marginLeft'));
            ...
            if(flag > 0){
                ...
                if(click_x>=0 && (netcaPDF_width - click_x - blank_width) >= blank_width){
    ...
    NetcaSignAPI.seal_SignSealPosition = function(){
        ...
        var xPos = parseInt(NetcaSignAPI.getMappingZoom(click_x - sealImage_halfWidth -10));
        var yPos = parseInt(NetcaSignAPI.getMappingZoom(click_y - sealImage_halfheight - 10) );//-10 paddingTop + marginTop
        ...
        netca_SignatureCreator_PdfSignSealPosition(srcFile, srcBytes, destFile, certEncode, selMode, signFieldText, sealImageEncode,
            revInfoIncludeFlag, pageNum, xPos, yPos, width, height,
            tsaUrl, tsaUsr, tsaPwd, tsaHashAlgo,
            NetcaSignAPI.SignatureCreatorSuccessCallBack, NetcaSignAPI.SignatureCreatorFailedCallBack);
    ...
    NetcaSignAPI.SignatureCreatorSuccessCallBack = function(res){
        ...
        layer.open({
            ...
            yes: function(index){
                //按钮确定的回调
                NetcaSignAPI.signedDataVerify('',signValue,NETCA_PKI_SINGEDDATA_VERIFY_LEVEL_VERIFY_CERTPATH_REVOKE);
    ...
    NetcaSignAPI.resetTheOriginalState = function(index){
        ...
        layer.close(index);
        NetcaSignAPI.netcaLoadPDF(NetcaSignAPI.scale, NetcaPDFSeal.getSignPDFBytes());
   ...
   NetcaSignAPI.openPDF = function(byteBase64){
           ...
           for (var i = 1; i < pdf.numPages + 1; i++) {
                ...
                void (function(pageNumber) {
                    pdf.getPage(pageNumber).then(function (page) {
                        ...
                        if ($('#netcaPDF').length)
                            $('#netcaPDF').scrollTop(pdfscrollTop);
    ...
    /**时间戳*/
    NetcaSignAPI.NetcaSetTimeStamp = function(tsaUrl,tsaUsr,tsaPwd,tsaHashAlgo){
    ...
    NetcaSignAPI.NetcaGetZoomScale = function(){
        ...
        for(var i=0;i<netca_scaleSelect_option.length;i++){
            ...
            if(netca_scaleSelect_option[i].value < NetcaSignAPI.scale){
                NetcaSignAPI.zoomInScale.push(Number(netca_scaleSelect_option[i].value));
            }
        }
    ...
    */
    NetcaSignAPI.NetcaZoomOutInPDF = function(condition){
        ...
        if(condition){
            for(var j=0;j<condition.length;j++){
                if(zoomFlag === j){
                    NetcaSignAPI.netcaLoadPDF(Number(condition[j]), NetcaPDFSeal.getSignPDFBytes());
    ...
    NetcaSignAPI.goTofixedPagePDF = function(event){
        ...
        if(Number(netcapageNumber.value) <= Number(netcanumPages.innerText)){
            if(event.keyCode === 13){
                netcaPDF1.scrollTop(($('canvas').outerHeight() - 7) * (Number(netcapageNumber.value) - 1));
    ...
    NetcaSignAPI.initUI = function(){
        ...
        if (NetcaUtils.isUserManualSignUI()) {
            ...
            netca_scaleSelect_option = netca_scaleSelect.getElementsByTagName('option');
             ...
             * */
            document.getElementById('netcasign').addEventListener('click', NetcaSignAPI.getCertList);
            document.getElementById('netcaVerfity').addEventListener('click',function(){NetcaSignAPI.signedDataVerify('',NetcaPDFSeal.SetPDFBytes,NETCA_PKI_SINGEDDATA_VERIFY_LEVEL_VERIFY_CERTPATH_REVOKE,true);});
            document.getElementById('netcazoomOut').addEventListener('click', function () { NetcaSignAPI.NetcaZoomPDFPage('zoomOut') });
            document.getElementById('netcazoomIn').addEventListener('click', function () { NetcaSignAPI.NetcaZoomPDFPage('zoomIn') });
            document.getElementById('netcaprevious').addEventListener('click', NetcaSignAPI.NetcaGoToPrevPage);
            document.getElementById('netcanext').addEventListener('click', NetcaSignAPI.NetcaGoToNextPage);
            document.getElementById('netcainvalidClose').addEventListener('click', function(){NetcaSignAPI.closeSignedDataVerifyInfo('netcainvalidClose')});
            document.getElementById('netcaclose2').addEventListener('click', function(){NetcaSignAPI.closeSignedDataVerifyInfo('netcaclose2')});
            document.getElementById('netcainvalidBtn').addEventListener('click', NetcaSignAPI.displaySignedDataVerifyInfo);


            /**input 页数keydown事件*/
            netcapageNumber.addEventListener('keydown', NetcaSignAPI.goTofixedPagePDF);
            ...
            $('#NetcaToolBar').bind('click', function (event) {
                ...
                if(event.target === event.currentTarget){
                    ...
                    if (Number(netcapageNumber.value) > Number(netcanumPages.innerText) || Number(netcapageNumber.value) <= 0 || isNaN(Number(netcaPageNum.val()))) {
                        netcaPDF1.scrollTop(0);
                        netcapageNumber.value = 1;
                    }
                    if (netcaPDF1.scrollTop() !== ($('canvas').outerHeight() - 7) * (Number(netcaPageNum.val()) - 1)) {
                        netcaPDF1.scrollTop(($('canvas').outerHeight() - 7) * (Number(netcaPageNum.val()) - 1));
    ...
    NetcaSignAPI.signedDataVerify = function(srcFile, srcBytes, level,params){
        if(params){
            if(netcasignedDataVerify.css('display') === 'none' || (netcasignedDataVerify.css('display') === 'none' && netcaDisplaySignedDataVerify.css('display') === 'block')){
                netca_signatureVerifierVerifyPDF(srcFile, srcBytes, level, NetcaSignAPI.SuccessSignedDataVerifyCallBack, NetcaSignAPI.FailedSignedDataVerifyCallBack)
            }
        } else{
            netca_signatureVerifierVerifyPDF(srcFile, srcBytes, level, NetcaSignAPI.SuccessSignedDataVerifyCallBack, NetcaSignAPI.FailedSignedDataVerifyCallBack)
        }
        ...
        for(var i=0;i<NetcaSignAPI.VerifyInfos.length;i++){
            (function(i){
                ...
                htmlDOM +=
                    ...
                    '                            <span class="netcasubjectCN"><span><img src="ui/image/'+icon+'.png" alt=""></span>'+NetcaSignAPI.VerifyInfos[i].signerName+'</span>\n' +
                    '                            <div class="netcasignContent">\n' +
                    '                                <div class="netcasignTip"><span>'+NetcaSignAPI.VerifyInfos[i].status1+'</span> <input type="button" value="详细" class="netcadetailBtn"><input type="hidden" value="'+NetcaSignAPI.VerifyInfos[i].certEncode+'" class="netcahiddenBtn"></div>\n' +
                    '                                <div class="netcasignTime">\n' +
                    '                                    <p>域名:'+NetcaSignAPI.VerifyInfos[i].fieldName+'</p>\n' +
                    '                                    <p><span> 签名时间:</span><br><span> '+NetcaSignAPI.VerifyInfos[i].signDate+'</span></p>\n' +
                    '                                    <p class="hasTsa"><span> '+NetcaSignAPI.VerifyInfos[i].hasTsaText+'</span><span>（'+NetcaSignAPI.VerifyInfos[i].isTsaValidText+'）</span><br><span> '+NetcaSignAPI.VerifyInfos[i].tsaDate+'</span></p>\n' +
                    '                                    <p class="hasRevoke"><span> '+NetcaSignAPI.VerifyInfos[i].hasRevokeText+'</span><span>（'+NetcaSignAPI.VerifyInfos[i].revokeStatus+'）</span></p>\n' +
                    '                                </div>\n' +
                    '                            </div>\n' +
                    '                        </div>';
            })(i);
        ...
        for(var i=0;i<btn_fold.length;i++){
            ...
            btn_fold[i].onclick = function () {
                if($('.netcasignTime').eq(this.index).css('display') === 'none'){
                    ...
                    $('.btn_fold_img').eq(this.index).attr('src','ui/image/btn_fold.png')
                }else{
                    $('.netcasignTime').eq(this.index).css({"display":"none"});
                    $('.btn_fold_img').eq(this.index).attr('src','ui/image/btn_unfold.png')
                }
            }
        }
    };
    ...
    NetcaSignAPI.getSignatureFieldInfo = function(signBytes){
        ...
        netca_getSignatureFieldInfo('',signBytes,function(res){
            if(res.signFields){
                for(var i=0;i<res.signFields.length;i++){
                    (function(i){

                        res.signFields[i].xPos = res.signFields[i].xPos * NetcaSignAPI.scale;
                        res.signFields[i].yPos = res.signFields[i].yPos * NetcaSignAPI.scale;

                        if(res.signFields[i].pageNum > 1){
                            res.signFields[i].yPos = ((res.signFields[i].pageNum - 1) * $('canvas').eq(pageNum-1).outerHeight(true) )+ res.signFields[i].yPos;
                        }

                        res.signFields[i].yPos = res.signFields[i].yPos + 9;
                        res.signFields[i].xPos = res.signFields[i].xPos + parseInt($('canvas').eq(pageNum-1).css('marginLeft')) + 9;
                        res.signFields[i].width = res.signFields[i].width * NetcaSignAPI.scale;
                        res.signFields[i].height = res.signFields[i].height * NetcaSignAPI.scale;

                        $('#netcaPDF').append(' <div class="netcafieldInfo" style="position:absolute;z-index:99;width:'+res.signFields[i].width+'px;height:'+res.signFields[i].height+'px;left:'+res.signFields[i].xPos+'px;top:'+res.signFields[i].yPos+'px;"></div>')
                ...
                if(filedFlag){
                    ...
                    $('#btn_discard').click(function(){
                        // -1表示最后的一个签名/章
                        NetcaSignAPI.signatureVerifierUndoPDF(NetcaPDFSeal.getSignPDFBytes(),-1);
                    });
                }
    ...
    NetcaSignAPI.signatureVerifierUndoPDF = function(_signBytes, _index){
        layer.confirm('是否需要撤销选定的签章？',{
            ...
            layer.close(index);
            netca_signatureVerifierUndoPDF('',_signBytes, _index,function(res){
    ...
    NetcaSignAPI.getSealConfigInfo = function() {
        netca_getSealConfigInfo(function(res){
            ...
            //验证签名级别
            NETCA_PKI_SINGEDDATA_VERIFY_LEVEL_VERIFY_CERTPATH_REVOKE = res.verify.verifySignLevel;
...
var NetcaPDFSeal = (function(){
    ...
    * */
    ...
    /**
     * 环境检测*/
    NetcaPDFSeal.checkKeyxAndSealTool = function(params1,params2,successCallBack) {
        netca_getClientVersionInfo(function(res){
            if(versionCompare(res.Version,params1))
            {
                netca_getSealClientVersion(function(res){
...
function versionCompare(Str1,Str2) {
    ...
    if(!nStr1 || !nStr2){ alert("您输入的两个版本号为空"); return true; }
