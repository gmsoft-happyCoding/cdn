...
var createOrFindInTop__default = /*#__PURE__*/_interopDefaultLegacy(createOrFindInTop);
...
function _objectSpread(target) {
  for (var i = 1; i < arguments.length; i++) {
    ...
    if (typeof getOwnPropertySymbols === 'function') {
      ownKeys = ownKeys.concat(getOwnPropertySymbols(source).filter(function (sym) {
        return getOwnPropertyDescriptor(source, sym).enumerable;
      }));
    }
...
var SET$1 = '@@THEME/SET';
var NAMESPACE$1 = 'theme';
var STORE_KEY$1 = "__@" + NAMESPACE$1 + "@__";
var CHANGE_KEY$1 = "__@" + NAMESPACE$1 + "Change@__";
function createThemePlugin() {
    var _a;
    var eventBus = createOrFindInTop__default['default']();
    var initialState = {
        rowProps: { gutter: 24 },
    ...
    try {
        if (top !== self) {
            if (top[STORE_KEY$1])
                initialState = top[STORE_KEY$1];
        }
        else {
            top[STORE_KEY$1] = initialState;
        }
    }
    catch (e) {
        // do nothing
    }
    var extraReducers = (_a = {},
        _a[NAMESPACE$1] = function (state, _a) {
            if (state === void 0) { state = initialState; }
            var type = _a.type, payload = _a.payload;
            switch (type) {
                case SET$1:
                    return __assign$1(__assign$1({}, state), payload);
                default:
                    return state;
            }
        },
    ...
    try {
        /**
         * 在宿主环境
         * 1. 保存 state 到 window[STORE_KEY]
         * 2. 发送事件通知子应用主题改变了
         */
        if (top === self) {
            var onAction = function (store) { return function (next) { return function (action) {
                if (action.type === SET$1) {
                    var state = __assign$1(__assign$1({}, store.getState()[NAMESPACE$1]), action.payload);
                    /**
                     * 保存 state 到 window[storeKey]
                     */
                    top[STORE_KEY$1] = state;
                    /**
                     * 发送事件通知子应用主题改变了
                     */
                    eventBus.emit(CHANGE_KEY$1, state);
                }
                next(action);
            }; }; };
            return { onAction: onAction, extraReducers: extraReducers };
        }
        /**
         * 在子应用中
         * 接受应用主题改变事件通知, 修改 state
         */
        var themeEnhancer = function (storeCreator) { return function (reducer, preloadedState, enhancer) {
            var store = storeCreator(reducer, preloadedState, enhancer);
            eventBus.on(CHANGE_KEY$1, function (payload) {
                store.dispatch({ type: SET$1, payload: payload });
            });
            return store;
        }; };
        return { extraReducers: extraReducers, extraEnhancers: [themeEnhancer] };
    }
    catch (e) {
        return {
            extraReducers: extraReducers,
...
function getTokenByCookie(_a) {
    var _b = _a.authCookieKey, authCookieKey = _b === void 0 ? 'Auth' : _b, _c = _a.authScheme, authScheme = _c === void 0 ? 'Auth' : _c;
    /**
     * 优先从 cookie 中获取 auth
     */
    if (document.cookie && document.cookie.length > 0) {
        ...
        if (cookies && cookies.length > 0) {
            var cookieAuth = cookies.find(function (cookie) { return cookie.startsWith(authCookieKey); });
            var auth = cookieAuth ? cookieAuth.split('=')[1] : null;
            if (auth)
...
function getTokenByQuery(_a) {
    var _b = _a.authScheme, authScheme = _b === void 0 ? 'Auth' : _b, _c = _a.queryByWindow, queryByWindow = _c === void 0 ? window.top || window : _c, _d = _a.authQueryKey, authQueryKey = _d === void 0 ? '__auth__' : _d, _e = _a.tokenQueryKey, tokenQueryKey = _e === void 0 ? '__token__' : _e, _f = _a.tokenScheme, tokenScheme = _f === void 0 ? 'Bearer' : _f;
    var queryObject = new URLSearchParams(queryByWindow.location.search);
    /**
     * 其次从 query 中获取 auth
     */
    if (authQueryKey) {
        var queryAuth = queryObject.get(authQueryKey);
        if (queryAuth && typeof queryAuth === 'string')
            return queryAuth && authScheme ? authScheme + " " + queryAuth : queryAuth;
    }
    /**
     * 再次从 query 中获取 token
     */
    if (tokenQueryKey) {
        var queryToken = queryObject.get(tokenQueryKey);
        if (queryToken && typeof queryToken === 'string')
            return queryToken && tokenScheme ? tokenScheme + " " + queryToken : queryToken;
    }
    return null;
}
/**
 * 优先从高到低
 * 1. 从 cookie 中获取 auth
 * 2. 从 url query 中获取 auth
 * 3. 从 url query 中获取 token
 */
var getToken = function (_a) {
    var _b = _a.authCookieKey, authCookieKey = _b === void 0 ? 'Auth' : _b, _c = _a.authScheme, authScheme = _c === void 0 ? 'Auth' : _c, _d = _a.queryByWindow, queryByWindow = _d === void 0 ? window.top || window : _d, _e = _a.authQueryKey, authQueryKey = _e === void 0 ? '__auth__' : _e, _f = _a.tokenQueryKey, tokenQueryKey = _f === void 0 ? '__token__' : _f, _g = _a.tokenScheme, tokenScheme = _g === void 0 ? 'Bearer' : _g, _h = _a.authQueryStrategy, authQueryStrategy = _h === void 0 ? 'COOKIE_FIRST' : _h;
    try {
        if (authQueryStrategy === 'COOKIE_FIRST') {
            return (getTokenByCookie({ authCookieKey: authCookieKey, authScheme: authScheme }) ||
                getTokenByQuery({ authScheme: authScheme, queryByWindow: queryByWindow, authQueryKey: authQueryKey, tokenQueryKey: tokenQueryKey, tokenScheme: tokenScheme }) ||
                null);
        }
        if (authQueryStrategy === 'URL_QUERY_FIRST') {
            return (getTokenByQuery({ authScheme: authScheme, queryByWindow: queryByWindow, authQueryKey: authQueryKey, tokenQueryKey: tokenQueryKey, tokenScheme: tokenScheme }) ||
                getTokenByCookie({ authCookieKey: authCookieKey, authScheme: authScheme }) ||
                null);
        }
        return null;
    }
...
var STORE_KEY = "__@" + NAMESPACE + "@__";
var CHANGE_KEY = "__@" + NAMESPACE + "Change@__";
var ME_URL = '/gateway/v1/user?requireAvatar=true';
var makePlatInfoUrl = function (djcGatewayBaseUrl) {
    return djcGatewayBaseUrl + "/djcsupport/domainname/getPlatformConfigure";
};
function createGlobalContextPlugin(opts) {
    var _a;
    if (opts === void 0) { opts = {}; }
    var eventBus = createOrFindInTop__default['default']();
    var initialState = { env: { inApp: false, hostName: opts.hostName } };
    /**
     * 子应用和宿主环境保持相同的 state
     * 尝试使用顶层窗口的主题变量初始化
     */
    try {
        if (top && top !== self) {
            if (top[STORE_KEY])
                initialState = top[STORE_KEY];
            /**
             * 标记子应用
             */
            initialState.env.inApp = opts.appName || true;
        }
        else if (top)
            top[STORE_KEY] = initialState;
    }
    catch (e) {
        // do nothing
    }
    var extraReducers = (_a = {},
        _a[NAMESPACE] = function (state, _a) {
            if (state === void 0) { state = initialState; }
            var type = _a.type, payload = _a.payload;
            switch (type) {
                case SET:
    ...
    try {
        ...
        if (top === self) {
            var onAction = function (store) { return function (next) { return function (action) {
                if (action.type === SET) {
                    var state = __assign(__assign({}, store.getState()[NAMESPACE]), action.payload);
                    /**
                     * 保存 state 到 window[STORE_KEY]
                     */
                    if (top)
                        top[STORE_KEY] = state;
                    /**
            ...
            var loadDataEnhancer = function (storeCreator) { return function (reducer, preloadedState, enhancer) {
                var store = storeCreator(reducer, preloadedState, enhancer);
                /**
                 * 尝试获取当前用户鉴权 token
                 */
                var token = getToken(opts.getTokenArgs || {});
                /**
                 * 设置请求头
                 */
                var axiosHeaders = token
                    ? {
                        Authorization: token,
                    }
                    : undefined;
                ...
                if (opts.djcGatewayBaseUrl) {
                    ...
                    axios__default['default']({
                        ...
                        // eslint-disable-next-line no-console
                        console.log('加载平台信息失败', error);
                    });
                }
                else {
                    // eslint-disable-next-line no-console
                    console.error('dva-global-context-plugin: 参数 djcGatewayBaseUrl 未指定, 平台信息无法加载');
                }
                return store;
            }; };
            return { onAction: onAction, extraReducers: extraReducers, extraEnhancers: [loadDataEnhancer] };
        }
        /**
         * 在子应用中
         * 接受改变事件通知, 修改 state
        ...
        var handleChangeEnhancer = function (storeCreator) { return function (reducer, preloadedState, enhancer) {
            var store = storeCreator(reducer, preloadedState, enhancer);
            eventBus.on(CHANGE_KEY, function (payload) {
                store.dispatch({ type: SET, payload: payload });
            });
            return store;
        }; };
        return { extraReducers: extraReducers, extraEnhancers: [handleChangeEnhancer] };
    }
    catch (e) {
        // eslint-disable-next-line no-console
        console.log('createGlobalContextPlugin error:', e);
        return {
            extraReducers: extraReducers,
        };
    }
...
function createStateContainer(_a) {
     ...
     */
    var createOpts = history
        ? {
            initialReducer: {
                routing: reactRouterRedux.routerReducer,
            },
            setupMiddlewares: function (middlewares) {
                return __spreadArray([reactRouterRedux.routerMiddleware(history)], middlewares);
            },
